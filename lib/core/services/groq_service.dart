  ...
  GroqService() {
    ...
    _conversationHistory.add(
      ChatMessage(
        role: 'system',
        content:
            '''Tu es TikiTaka, l'assistant officiel de la CAN 2025 (Coupe d'Afrique des Nations) au Maroc.
      ...
      if (response.statusCode == 200) {
        ...
        if (functionResponse != null) {
          _conversationHistory.removeWhere(
            (m) =>
                m.role == 'system' &&
                m.content.contains('Voici les données actuelles'),
          );
        }
  ...
  Future<String> _getLiveMatchesInfo() async {
    try {
      ...
      for (final m in matches) {
        buffer.writeln(
          '${m.homeTeam.flagEmoji} ${m.homeTeam.name} ${m.homeScore ?? 0} - ${m.awayScore ?? 0} ${m.awayTeam.name} ${m.awayTeam.flagEmoji}',
        );
  ...
  Future<String> _getUpcomingMatchesInfo() async {
    try {
      ...
      for (final m in matches.take(5)) {
        buffer.writeln(
          '${m.homeTeam.flagEmoji} ${m.homeTeam.name} vs ${m.awayTeam.name} ${m.awayTeam.flagEmoji}',
        );
        buffer.writeln(
          '📅 ${m.dateTime.day}/${m.dateTime.month}/${m.dateTime.year} à ${m.dateTime.hour}h${m.dateTime.minute.toString().padLeft(2, '0')}',
        );
        buffer.writeln('📍 ${m.stadium}, ${m.city}');
  ...
  Future<String> _getResultsInfo() async {
    try {
      ...
      for (final m in matches.take(5)) {
        ...
        buffer.writeln(
          '📅 ${m.dateTime.day}/${m.dateTime.month}/${m.dateTime.year}',
        );
        buffer.writeln('');
      }
      return buffer.toString();
  ...
  Future<String> _getTodayMatchesInfo() async {
    ...
    try {
      ...
      if (matches.isNotEmpty) {
        ...
        for (final m in matches) {
          final statusEmoji = m.status.isLive
              ? '🔴 EN DIRECT'
              : m.status.isFinished
              ? '✅ Terminé'
              : '⏰ À venir';
          ...
          buffer.writeln(
            '${m.homeTeam.flagEmoji} ${m.homeTeam.name} $scoreOrTime ${m.awayTeam.name} ${m.awayTeam.flagEmoji}',
          );
          buffer.writeln('📍 ${m.stadium}, ${m.city}');
          if (m.group != null) buffer.writeln('🏆 Groupe ${m.group}');
          buffer.writeln('');
    ...
    try {
      ...
      standings.forEach((group, teams) {
        ...
        for (final t in teams) {
          buffer.writeln(
            '  ${t['rank']}. ${t['team']} - ${t['points']} pts (${t['won']}V ${t['draw']}N ${t['lost']}D)',
          );
        }
        buffer.writeln('');
      });
...
Groupe F: Côte d'Ivoire, Cameroun, Gabon, Mozambique''';
  ...
  Future<String> generateMatchSummary(
    ...
    final prompt =
        '''Génère un résumé court et engageant du match de la CAN 2025:
